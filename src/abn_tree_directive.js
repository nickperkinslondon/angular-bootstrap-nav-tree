// Generated by CoffeeScript 1.4.0
var module;

module = angular.module('angularBootstrapNavTree', []);

module.directive('abnTree', function($timeout) {
  return {
    restrict: 'E',
    templateUrl: '../dist/abn_tree_template.html',
    scope: {
      treeData: '=',
      onSelect: '&',
      initialSelection: '='
    },
    link: function(scope, element, attrs) {
      var expand_all, expand_level, for_each_branch, on_treeData_change, select_branch, selected_branch, unselect_all, _ref, _ref1, _ref2, _ref3;
      if ((_ref = attrs.iconExpand) == null) {
        attrs.iconExpand = 'icon-plus';
      }
      if ((_ref1 = attrs.iconCollapse) == null) {
        attrs.iconCollapse = 'icon-minus';
      }
      if ((_ref2 = attrs.iconLeaf) == null) {
        attrs.iconLeaf = 'icon-chevron-right';
      }
      if ((_ref3 = attrs.expandLevel) == null) {
        attrs.expandLevel = '3';
      }
      expand_level = parseInt(attrs.expandLevel, 10);
      scope.header = attrs.header;
      if (!scope.treeData) {
        alert('no treeData defined for the tree!');
      }
      if (!(scope.treeData.length != null)) {
        if (treeData.label != null) {
          scope.treeData = [treeData];
        } else {
          alert('treeData should be an array of root branches');
        }
      }
      for_each_branch = function(f) {
        var do_f, root_branch, _i, _len, _ref4, _results;
        do_f = function(branch, level) {
          var child, _i, _len, _ref4, _results;
          f(branch, level);
          if (branch.children != null) {
            _ref4 = branch.children;
            _results = [];
            for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
              child = _ref4[_i];
              _results.push(do_f(child, level + 1));
            }
            return _results;
          }
        };
        _ref4 = scope.treeData;
        _results = [];
        for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
          root_branch = _ref4[_i];
          _results.push(do_f(root_branch, 1));
        }
        return _results;
      };
      for_each_branch(function(branch) {
        if (branch.children) {
          if (branch.children.length > 0) {
            return branch.children = branch.children.map(function(e) {
              if (typeof e === 'string') {
                return {
                  label: e,
                  children: []
                };
              } else {
                return e;
              }
            });
          }
        } else {
          return branch.children = [];
        }
      });
      for_each_branch(function(b, level) {
        b.level = level;
        b.expanded = b.level < expand_level;
        return b.uid = "" + Math.random();
      });
      expand_all = function() {
        return for_each_branch(function(branch) {
          return branch.expanded = true;
        });
      };
      unselect_all = function() {
        return for_each_branch(function(branch) {
          return branch.selected = false;
        });
      };
      selected_branch = null;
      select_branch = function(branch) {
        if (branch !== selected_branch) {
          unselect_all();
          branch.selected = true;
          selected_branch = branch;
          if (branch.onSelect != null) {
            return $timeout(function() {
              return branch.onSelect(branch);
            });
          } else {
            if (scope.onSelect != null) {
              return scope.onSelect({
                branch: branch
              });
            }
          }
        }
      };
      scope.user_clicks_branch = function(branch) {
        if (branch !== selected_branch) {
          return select_branch(branch);
        }
      };
      scope.tree_rows = [];
      on_treeData_change = function() {
        var add_all, root_branch, _i, _len, _ref4, _results;
        scope.tree_rows = [];
        add_all = function(level, branch, visible) {
          var child, child_visible, tree_icon, _i, _len, _ref4, _results;
          if (!(branch.expanded != null)) {
            branch.expanded = false;
          }
          if (branch.children.length === 0) {
            tree_icon = attrs.iconLeaf;
          } else {
            if (branch.expanded) {
              tree_icon = attrs.iconCollapse;
            } else {
              tree_icon = attrs.iconExpand;
            }
          }
          scope.tree_rows.push({
            level: level,
            branch: branch,
            label: branch.label,
            tree_icon: tree_icon,
            visible: visible
          });
          if (branch.children != null) {
            _ref4 = branch.children;
            _results = [];
            for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
              child = _ref4[_i];
              child_visible = visible && branch.expanded;
              _results.push(add_all(level + 1, child, child_visible));
            }
            return _results;
          }
        };
        _ref4 = scope.treeData;
        _results = [];
        for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
          root_branch = _ref4[_i];
          _results.push(add_all(1, root_branch, true));
        }
        return _results;
      };
      if (attrs.initialSelection != null) {
        for_each_branch(function(b) {
          if (b.label === attrs.initialSelection) {
            return select_branch(b);
          }
        });
      }
      return scope.$watch('treeData', on_treeData_change, true);
    }
  };
});
